...
$semester_reg_pay =$_SESSION["semester_reg_pay"];
    ...
    elseif($bank == "fisal"){
        ...
        if(mysqli_num_rows($display_info_account) == 0 ){
         echo "<script>alert('SORRY, Erorr In Information Account');
         window.location.href='online_pay.php'</script>";
        }
        elseif(mysqli_num_rows($display_info_account) == 1){
            //رصيد حساب العميل
            $clint_balance = mysqli_fetch_array($display_info_account)["balance"];
            //عرض رصيد الحساب الخاص بالادارة
            $display_balance_main_account = mysqli_query($connection ,"select * from safe_monye");
            $balance_main = mysqli_fetch_array($display_balance_main_account)["balance"];
            if($total_price > 70000000){
              echo "<script>alert('SORRY, Not Enough Balance In Account');
                 window.location.href='online_pay.php';</script>";
            }
            else{
                ...
                if($update_account_clinet){
                 $update_account_main =mysqli_query($connection , "update safe_monye set balance=$balance_main+$total_price");
                 if($update_account_main){
                 $insert_data_book = mysqli_query($connection , "insert into request_events (name,phone_number,address,type_event,date_event,time_event,type_chairs,n_chairs,type_tables,n_tables,n_condition,type_hall,meal_foot,n_meal_foad,total_sum,warranty_value,total_of_all,payment,date_book,time_book)
                  value ('$name','$phone','$address','$type','$date_event','$time_event','$type_chairs','$number_chairs','$type_tables','$number_tables','$Condition','$type_hall','$meal_food','$number_meal','$total_summation','$warranty_value','$total_price','done','$date','$time')");
                  if($insert_data_book){
                      if(session_destroy()){
                     echo "<script>alert('Successfully Book');
