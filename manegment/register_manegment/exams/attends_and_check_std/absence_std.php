...
$dispaly_data_std_for_check_if_exisit = mysqli_query($connection, "select * from submit_std_and_result_subjects where unv_id='$unv_id' && name_std='$name_std' && type_certifcate_unv='$type_certificate' && department='$department' && batch='$batch' && study_year='$study_year' && semester='$semester' && name_subject='$name_subject' && type_exam='$type_exam' && come_to_exam_in_first_time='yes'");
...
elseif(isset($_POST["submit_absence"])){
    ...
    $display_hours_subject_for_insert = mysqli_query($connection , "select number_hours_subject from subjects where name_subject='$name_subject' ");
        ...
        if($display_hours_subject_for_insert){
            $insert_data_std = mysqli_query($connection , "insert into submit_std_and_result_subjects (unv_id,name_std,type_certifcate_unv,department,batch,study_year,semester,name_subject,come_to_exam_in_first_time , type_exam , date ,hour,status_exam)
             value ('$unv_id2','$name_std2','$type_certificate2','$department2','$batch','$study_year','$semester','$name_subject2','no','$type_exam','$date','$hour','sub') ");
         if($insert_data_std){
   ...
   /* $dispaly_password_for_submit = mysqli_query($connection , "select password from students where password='$password_std' && unv_id='$unv_id' ");
    if(mysqli_num_rows($dispaly_password_for_submit) == 1) {
        $insert_data_std = mysqli_query($connection , "insert into submit_std_and_result_subjects (unv_id,name_std,type_certificate,department,batch,study_year,semester,name_subject,come_to_exam_in_first_time , type_exam , date ,hour)
             value ('$unv_id2','$name_std2','$type_certificate2','$department2','$batch','$study_year','$semester','$name_subject2','yes','$type_exam','$date','$hour') ");
         if($insert_data_std){
        //echo "<script>alert('SUCCESSEFULY')</script>";
         header("location: attends_and_check_std.php?unv_id=$unv_id&name_std=$name_std&name_subject=$name_subject&type_certificate=$type_certificate&department=$department&batch=$batch&study_year=$study_year&semester=$semester&type_exam=$type_exam");
         }
...
if(mysqli_num_rows($dispaly_data_std_for_check_if_exisit) == 1){
    ...
    if(isset($_POST["submit_password"])){
        ...
        $dispaly_password_for_submit = mysqli_query($connection , "select password from students where password='$password_std' && unv_id='$unv_id' ");
        if(mysqli_num_rows($dispaly_password_for_submit) == 1) {
            $update_data = mysqli_query($connection , "update submit_std_and_result_subjects set come_to_exam_in_second_time='no' ,type_exam2='sub_exam' where unv_id='$unv_id' && name_std='$name_std' && type_certifcate_unv='$type_certificate' && department='$department' && batch='$batch' && study_year='$study_year' && semester='$semester' && name_subject='$name_subject'");
             if($update_data){
            //echo "<script>alert('SUCCESSEFULY')</script>";
             header("location: attends_and_check_std.php?unv_id=$unv_id&name_std=$name_std&name_subject=$name_subject&type_certificate=$type_certificate&department=$department&batch=$batch&study_year=$study_year&semester=$semester&type_exam=$type_exam");
             }
