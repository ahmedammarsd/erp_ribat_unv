...
$display_username = mysqli_query($connection , "select full_name from employes"); 
...
<div class="side-menu">
    ...
    <form action="" method="post">  
        <div class="roww">  
            ...
            <label for="" class="lead">Select Type Of Report</label>
            <select name="reports" id="" class="form-select">
                <option value="none">---SELECT REPORTS---</option>
                <option value="exp">Expenses</option>
                <option value="loans">Loans</option>
                <option value="mustahqat">Musthaqat</option> 
                <option value="fees_std">Student Registaration Fee</option>
                <option value="all">Total Reports</option> 
            </select>
            </div>
            <div class="form-group">
            <label for="" class="lead">Select Date from :</label>
            <input type="date" name="dateone" id="" class="form-control">
          </div>
            <div class="form-group">
            <label for="" class="lead">To : </label>
            <input type="date" name="datetwo" id="" class="form-control">
            </div>
         </div>
     <!--   <select name="username" id="">
            ...
            /*
           while($rowusername = mysqli_fetch_array($display_username)){
               $name = $rowusername["full_name"];
    ...
    if(isset($_POST["ser"])){
      ...
      //  $username1 = $_POST["username"];
        ...
        elseif($report_type === "exp"){
           ...
           ";
           $display_data_from_exp = mysqli_query($connection , "select username,type_exp,value_exp,exp_from,date_of_exp from expenses where date_of_exp between '$date_one' and '$date_two'");
        ...
        echo "</table>" ;
        echo "<br>";
        $display_total_from_exp = mysqli_query($connection , "select sum(value_exp) AS total_exp from expenses where date_of_exp between '$date_one' and '$date_two'");
        $row_exp = mysqli_fetch_array($display_total_from_exp);
        $total_exp = $row_exp["total_exp"];
        echo "Total Loans :" . $total_exp . "<br>";
    ...
    elseif($report_type === "loans"){
       ...
       ";
       $display_data_from_loans = mysqli_query($connection , "select name_of_take,type_of_job,phone_number,value_of_loans,username,date from loans where date between '$date_one' and '$date_two'");
    ...
    echo "</table>" ;
    echo "<br>";
        $display_total_from_loans = mysqli_query($connection , "select sum(value_of_loans) AS total_loans from loans where date between '$date_one' and '$date_two'");
        $row_loans = mysqli_fetch_array($display_total_from_loans);
        $total_loans = $row_loans["total_loans"];
        echo "Total Loans :" . $total_loans . "<br>";
...
elseif($report_type === "mustahqat"){
   ...
   ";
   $display_data_from_mustahqat = mysqli_query($connection , "select name_of_take , type_jop ,value_mustahqat ,type_safe ,user_name ,date_of_mus ,hours_of_mus from mustahqat where date_of_mus between '$date_one' and '$date_two'");
...
echo "</table>" ;
echo "<br>";
$display_total_from_mustahqat = mysqli_query($connection , "select sum(value_mustahqat) AS total_mus from mustahqat where date_of_mus between '$date_one' and '$date_two'");
$row_mus = mysqli_fetch_array($display_total_from_mustahqat);
$total_mus = $row_mus["total_mus"];
echo "Total Mustahqat :" . $total_mus . "<br><br>";
}
elseif($report_type === "fees_std"){
   ...
   ";
   $display_data_fees_std = mysqli_query($connection , "select * from std_reg_fees where date between '$date_one' and '$date_two'");
...
echo "</table>" ;
    echo "<br>";
    $display_total_from_fees = mysqli_query($connection , "select sum(total_pay) AS total_fees from std_reg_fees where date between '$date_one' and '$date_two'");
...
echo "<br><br><br>";
echo "<label class='lead'>Total Expenses :</label>" . $total_exp . "<br>";
echo "<label class='lead'>Total Loans :</label>" . $total_loans . "<br>";
echo "<label class='lead'>Total Mustahqat :</label>" . $total_mus . "<br><br>";
echo "<div class='alert alert-primary'>Total Fees :" . $total_fees . "</div>";
echo "<label class='lead'>Total :</label>";
echo $total_exp + $total_loans + $total_mus + $total_fees ."<br>";
} 
 }  
    ?>
